...
func TestCacheLastWriteWins(t *testing.T) {
    ...
    for i := 0; i < 100; i++ {
        key := "test"
        a := lawtest.IntGen(1, 1000)()
        b := lawtest.IntGen(1, 1000)()
...
func TestVectorAddition(t *testing.T) {
    ...
    lawtest.Associative(t, add, PointGen())
    lawtest.Commutative(t, add, PointGen())
    lawtest.Identity(t, add, Point{0, 0}, PointGen())
}
```
...
- Real-world hash collision bugs in caching systems ✓
